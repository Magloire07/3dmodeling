...
enum {
	...
	BUFFER_INDICES_TRIANGLES, BUFFER_INDICES_EDGES, BUFFER_INDICES_VERTICES, BUFFER_FACENORMALS_FORDRAWING
};
enum { VAO_TRIANGLES_NORMSPERVERTEX = 0, VAO_TRIANGLES_NORMSPERFACE, VAO_EDGES, VAO_VERTICES, VAO_NORMALS, VAO_FACENORMALS };
...
void makeBuffers(myMesh *input_mesh)
{
	...
	// Create buffer for face normals
	vector <GLfloat> face_normals_fordrawing;
	for (unsigned int i = 0; i < input_mesh->faces.size(); i++)
	{
		if (input_mesh->faces[i] == NULL || input_mesh->faces[i]->adjacent_halfedge == NULL) continue;
		
		// Compute face center
		float centerX = 0.0f, centerY = 0.0f, centerZ = 0.0f;
		int count = 0;
		myHalfedge *h = input_mesh->faces[i]->adjacent_halfedge;
		myHalfedge *start = h;
		do {
			centerX += (GLfloat)h->source->point->X;
			centerY += (GLfloat)h->source->point->Y;
			centerZ += (GLfloat)h->source->point->Z;
			count++;
			h = h->next;
		} while (h != start && h != NULL);
		...
		// Add face center
		face_normals_fordrawing.push_back(centerX);
		face_normals_fordrawing.push_back(centerY);
		face_normals_fordrawing.push_back(centerZ);
		
		// Add face center + normal
		face_normals_fordrawing.push_back(centerX + (GLfloat)(input_mesh->faces[i]->normal->dX / 10.0f));
		face_normals_fordrawing.push_back(centerY + (GLfloat)(input_mesh->faces[i]->normal->dY / 10.0f));
		face_normals_fordrawing.push_back(centerZ + (GLfloat)(input_mesh->faces[i]->normal->dZ / 10.0f));
	}
	...
	glBindBuffer(GL_ARRAY_BUFFER, buffers[BUFFER_FACENORMALS_FORDRAWING]);
	glBufferData(GL_ARRAY_BUFFER, face_normals_fordrawing.size() * sizeof(GLfloat), &face_normals_fordrawing[0], GL_STATIC_DRAW);
